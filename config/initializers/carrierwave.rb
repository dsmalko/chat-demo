if ENV['CLOUDCUBE_URL']
  ...
  CarrierWave.configure do |config|
    config.storage    = :aws
    config.aws_bucket = ENV['CLOUDCUBE_URL'].split("/").last # for AWS-side bucket access permissions config, see section below
    ...
    config.aws_credentials = {
      ...
      endpoint:          ENV['CLOUDCUBE_URL'].split("/")[0..-2].join("/"),
